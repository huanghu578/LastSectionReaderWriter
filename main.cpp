...
int main(){
    ...
    while (1) {
        ...
        if (readLen > 0) {
                ...
                if (to_lower(trim(serialRead)) == "read_tip" && current_setting.locked == LOCKED255) {
                ...
                if (to_lower(trim(serialRead)) == "read_count" && current_setting.locked == LOCKED255) {
                    serial.printf("%d\n", TYPE);
                    goto loop;
                }
                if (to_lower(trim(serialRead)) == "read_block" && current_setting.locked == LOCKED255) {
                    vector<string> parameters = split(serialRead, " ");  // 命令、index                    
                ...
                if (to_lower(trim(serialRead)) == "read_response" && current_setting.locked == LOCKED255) {
                    vector<string> parameters = split(serialRead, " ");  // 命令、index、query_code
                    ...
                    // WrongCommand("command error!");//因为此命令是close的，所以错误也不应该反馈
                    goto loop;
                }
                //write command
                if (to_lower(trim(serialRead)) == "write_dongle" &&
                    ...
                    if (parameters.size() == 4 &&
                        ...
                        current_setting.password_try_time = 0; //TODO：为减少写flash次数，密码正确时不写flash
                        goto loop;
                    }
                    if (parameters[1] !=
                        ...
                        write_init_setting();
                        serial.printf(wrong_password,current_setting.password_try_time);
                        goto loop;
                    }
                }
                if (to_lower(trim(serialRead)) == "write_init_setting" // && current_setting.locked == LOCKED255
                ) {//此处留下后门，在锁住也可以复位
                    write_init_setting();
                    // OkCommand("write_init_setting command OK!");//因为此命令是close的，所以不应该反馈
                    goto loop;
                }
                if (to_lower(trim(serialRead)) == "write_block" && current_setting.locked == LOCKED255) {
                    vector<string> parameters = split(serialRead, " ");
                    if (parameters.size() == 6) {  // 命令、密码、index、mode、max_try、seed                        
                        if (write_block(parameters[1], parameters[2],
